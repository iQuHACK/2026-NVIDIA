...
from .qkanGPT2LMHeadModel import GPT2LMHeadModel
...
class Transformer(LightningModule):
    ...
    def __init__(self, cfg, cost, loss="exp", numQPUs=1):
        ...
        self._starting_idx = torch.zeros(
            self.num_samples, 1, dtype=torch.int, device=get_device()
        )
        if loss == "exp":
            self.loss = ExpLogitMatching(cfg.energy_offset, self._label)
        else:
            self.loss = GFlowLogitMatching(
                cfg.energy_offset, get_device(), self._label, self
            )
        self._cost = cost
    ...
    def gather(self, idx, logits_base):
        ...
        """
        b_size = idx.shape[0]
        return torch.gather(logits_base, 2, idx.reshape(b_size, -1, 1)).reshape(
            b_size, -1
        )
    ...
    def computeCost(self, idx_output, pool, **kwargs):
        ...
        if not isinstance(res[0], float):
            raise RuntimeError("Invalid return type detected from user cost function.")
