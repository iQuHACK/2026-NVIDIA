...
def run_benchmark(runs: int = 100):
    ...
    print(f"{'Run':<5} {'N':<5} {'Best E':<10} {'Conv. Time':<12} {'Total Time':<12} {'Merit F.':<10} {'Generations':<12} {'Sequence':<20}")
    print("-" * 100)
    ...
    writer.writerow(["Run", "N", "Best E", "Conv. Time", "Total Time", "Merit F.", "Generations", "Sequence"])
    ...
    for run_idx in range(1, int(runs) + 1):
        for N in range(1,40):
            ...
            try:
                ...
                print(f"{run_idx:<5} {N:<5} {best_e:<10} {convergence_time:<12} {total_time:<12} {merit:<10} {total_gen:<12} {seq:<20}")
                writer.writerow([run_idx, N, best_e, convergence_time, total_time, merit, total_gen, seq])
                csv_file.flush()

            except subprocess.TimeoutExpired:
                print(f"{run_idx:<5} {N:<5} TIMEOUT")
                writer.writerow([run_idx, N, "TIMEOUT", "N/A", "N/A", "N/A", "N/A", ""])
                csv_file.flush()
            except Exception as e:
                print(f"{run_idx:<5} {N:<5} ERROR: {e}")
                writer.writerow([run_idx, N, "ERROR", "N/A", "N/A", "N/A", "N/A", ""])
                csv_file.flush()
