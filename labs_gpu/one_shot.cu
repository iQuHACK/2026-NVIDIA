...
// -------------------------------------------------------------------------
// Helper: Smart Binary Loader (Supports Partial Injection)
// -------------------------------------------------------------------------
void load_warm_start_partial(const char* filename, int8_t* d_pop, size_t max_bytes) {
    ...
    if (file_size > max_bytes) {
        std::cerr << "Warning: Warm start file is larger than population! Truncating to fit." << std::endl;
        file_size = max_bytes;
    }

    // Read file
    std::vector<int8_t> h_buffer(file_size);
    file.read((char*)h_buffer.data(), file_size);

    // Copy only the valid data to the GPU (leaving the rest of d_pop as initialized randoms)
    CUDA_CHECK(cudaMemcpy(d_pop, h_buffer.data(), file_size, cudaMemcpyHostToDevice));

    int num_injected = file_size / MAX_N;
    std::cout << ">>> Injected " << num_injected << " Elite Candidates from " << filename << std::endl;
    std::cout << ">>> Remaining " << (max_bytes / MAX_N) - num_injected << " are Random." << std::endl;
}
...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::cout << "Usage: ./labs_solver [N] [POP_SIZE] [ITERS=1M] [WARM_START_FILE]" << std::endl;
        return 0;
    }
