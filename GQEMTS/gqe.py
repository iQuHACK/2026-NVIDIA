...
class CosineScheduler(TemperatureScheduler):
    ...
    def get(self, iter):
        ...
        ) / 2 * math.cos(2 * math.pi * iter / self.frequency)
...
def get_default_config():
    ...
    cfg.trajectory_file_path = (
        "gqe_logs/gqe_trajectory.json"  # Path to save trajectory data
    )
    cfg.verbose = False
    return cfg


def __internal_run_gqe(
    temperature_scheduler: TemperatureScheduler, cfg: ConfigDict, model, pool, optimizer
):
...
def gqe(cost, pool, config=None, **kwargs):
    ...
    model = (
        Transformer(cfg, cost, loss="exp", numQPUs=numQPUs)
        if "model" not in kwargs
        else kwargs["model"]
    )
    optimizer = (
        torch.optim.AdamW(model.parameters(), lr=cfg.lr)
